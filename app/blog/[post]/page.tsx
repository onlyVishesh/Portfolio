...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${post.title}`,
    metadataBase: new URL(`https://onlyvishesh.vercel.app/blog/${post.slug}`),
    ...
    alternates: {
      canonical:
        post.canonicalLink ||
        `https://onlyvishesh.vercel.app/blog/${post.slug}`,
    },
    openGraph: {
      images:
        urlFor(post.coverImage?.image).width(1200).height(630).url() ||
        fallbackImage,
      url: `https://onlyvishesh.vercel.app/blog/${post.slug}`,
      title: post.title,
      description: post.description,
      type: "article",
